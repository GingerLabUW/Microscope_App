...
from .APD_StepperMotor import APD_StepperMotor
import serial
import sys

class APDStepperMotorHW(HardwareComponent):
	
	def setup(self):
		# Define your hardware settings here.
		# These settings will be displayed in the GUI and auto-saved with data files
		self.name = "apd_steppermotor"
	...
	def connect(self):
		...
		sm = self.apd_steppermotor = APDStepperMotor(port, baudrate)
	...
	def set_port(self):
		self.apd_steppermotor.port = self.settings["new_port"]
		self.settings.port.read_from_hardware()

	def set_baudrate(self):
		self.apd_steppermotor.baudrate = self.settings["new_baudrate"]
	...
	def abs_mov(self):
		if hasattr(self, "apd_steppermotor"):
			...
			self.apd_steppermotor.goto([x_abs_pos, y_abs_pos])
			self.read_position()
	...
	def rel_mov(self):
		if hasattr(self, "apd_steppermotor"):
			...
			self.apd_steppermotor.goto([x_rel_pos, y_rel_pos, "r"])
			self.read_position()
	...
	def disconnect(self):
		...
		if hasattr(self, "apd_steppermotor"):
			self.apd_steppermotor.close()
			del self.apd_steppermotor
			self.apd_steppermotor = None